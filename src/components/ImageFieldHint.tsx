...
export function ImageFieldHint({
  ...
  return (
    <div className="space-y-1 mt-1">
      {/* Recommended specs */}
      <div className="flex items-start gap-1.5 text-xs text-muted-foreground/35 leading-relaxed">
      ...
      {autoCrop && (
        <p className="text-[11px] text-muted-foreground/25 pl-[18px]">
          A imagem poderá ser ajustada automaticamente para o formato da área de exibição.
        </p>
      )}

      {note && (
        <p className="text-[11px] text-muted-foreground/25 pl-[18px]">{note}</p>
      )}
